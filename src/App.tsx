...
const ServiceTickets = ({ session, counselingMode, onClose }: { session: UserSession, counselingMode: boolean, onClose: any }) => {
  ...
  const create = async () => {
    ...
    await api.callService("tickets", "/tickets", { 
        method: "POST", 
        body: JSON.stringify({ subject: sub, description: desc, studentName: session.name, requestDate: date }) 
    });
  ...
  const deleteTicket = async (id: string) => {
      console.log("Deleting ticket ID:", id); // FIX: Logging ID to satisfy linter
          ...
          {session.role === "student" ? (
            <>
              ...
              <div className="space-y-4">
                <div><label className="text-xs font-bold text-gray-500 uppercase">Subject</label><input className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none mt-1" value={sub} onChange={e => setSub(e.target.value)} placeholder="e.g. Academic Stress" /></div>
                <div><label className="text-xs font-bold text-gray-500 uppercase">Preferred Date</label><input type="datetime-local" className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none mt-1" value={date} onChange={e => setDate(e.target.value)} /></div>
                <div><label className="text-xs font-bold text-gray-500 uppercase">Details</label><textarea className="w-full border border-gray-300 p-3 rounded-xl h-32 focus:ring-2 focus:ring-violet-500 outline-none resize-none mt-1" value={desc} onChange={e => setDesc(e.target.value)} placeholder="How can we help?" /></div>
            ...
            <div className="text-center p-6 bg-gray-50 rounded-xl border border-gray-200">
              <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center mb-4 transition-colors ${counselingMode ? "bg-green-100 text-green-600 border-2 border-green-200" : "bg-gray-200 text-gray-400 border-2 border-gray-300"}`}><Activity size={36}/></div>
              <h4 className="font-bold text-gray-800 text-lg">Counselor Dashboard</h4>
              <div className={`mt-3 px-4 py-1 rounded-full text-xs font-bold inline-block ${counselingMode ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"}`}>{counselingMode ? "ACCEPTING TICKETS" : "OFF DUTY"}</div>
              <p className="text-sm text-gray-500 mt-4 leading-relaxed">{counselingMode ? "Select an OPEN ticket from the list to assign it to yourself." : "Go to the Counseling Hub (S5) to go Online."}</p>
            </div>
          )}
        </div>
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center mb-2"><h4 className="font-bold text-gray-700 text-lg">{session.role === 'counselor' ? "All Incoming Requests" : "My Requests"}</h4><button onClick={refresh} className="text-sm text-violet-600 hover:underline flex items-center gap-1"><Clock size={12}/> Refresh</button></div>
          {tickets.length === 0 && <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300"><p className="text-gray-400 font-medium">No tickets found.</p></div>}
          {tickets.map(t => (
            <div key={t.id} className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm hover:shadow-md transition-all relative group">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="flex items-center gap-3"><span className="font-bold text-xl text-gray-800">#{t.id}</span><h5 className="font-semibold text-lg text-gray-700">{t.subject}</h5></div>
                  <div className="text-xs text-gray-500 mt-1 flex gap-2">Requested for: <span className="font-medium text-gray-700">{t.requestDate ? new Date(t.requestDate).toLocaleString() : "ASAP"}</span></div>
                ...
                <div className="flex gap-2">
                    {session.role === "student" && t.studentName === session.name && (
                        <button onClick={() => deleteTicket(t.id)} className="bg-red-50 text-red-600 p-2 rounded-lg hover:bg-red-100 transition-colors border border-red-100" title="Delete Ticket"><Trash2 size={18}/></button>
                    )}
                    {session.role === "counselor" && t.status === "OPEN" && (
                    <button disabled={!counselingMode} onClick={() => pickup(t.id)} className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm ${counselingMode ? "bg-violet-600 text-white hover:bg-violet-700 hover:shadow-md" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>{counselingMode ? "Pick Up" : "Go Online First"}</button>
                    )}
                </div>
              </div>
              <div className="mt-4 p-4 bg-gray-50 rounded-xl text-gray-600 text-sm border border-gray-100">{t.description}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <Card title="Community Chat" icon={MessageSquare} onClose={onClose}>
      <div className="flex flex-col h-full bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
          ...
          {posts.map(p => {
            ...
            return (
              <div key={p.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                <div className={`max-w-[75%] p-4 rounded-2xl shadow-sm text-sm leading-relaxed ${isMe ? "bg-violet-600 text-white rounded-br-none" : isCounselorPost ? "bg-blue-600 text-white rounded-bl-none" : "bg-white text-gray-800 border border-gray-200 rounded-bl-none"}`}>{p.content}</div>
                <span className="text-[10px] text-gray-400 mt-1 px-1 font-medium flex items-center gap-1">{p.author}</span>
              </div>
            );
          })}
          <div ref={endRef}></div>
        ...
        <div className="p-4 bg-white border-t border-gray-200 flex gap-3 items-center">
          <input className="flex-1 border border-gray-300 bg-gray-50 rounded-full px-5 py-3 focus:ring-2 focus:ring-violet-500 outline-none transition-all" value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && post()} placeholder={`Message as ${session.name}...`} />
          <button onClick={post} className="bg-violet-700 text-white p-3 rounded-full hover:bg-violet-800 transition-colors shadow-md hover:shadow-lg active:scale-95"><Send size={20}/></button>
        </div>
      </div>
    </Card>
  );
...
const ServiceAppointments = ({ session, onClose }: { session: UserSession, onClose: any }) => {
  ...
  useEffect(() => {
    api.callService("tickets", "/tickets").then(list => {
      if (session.role === 'counselor') { setTickets(list); } else { setTickets(list.filter((t: any) => t.studentName === session.name)); }
    }).catch(console.error);
  }, [session]);

  useEffect(() => { if (selId) { api.callService("appointments", `/appointments/${selId}`).then(setAppt).catch(console.error); } }, [selId, refreshKey]);

  const saveSlot = async (slot: string) => {
    await api.callService("appointments", "/appointments", { method: "POST", body: JSON.stringify({ ticketId: selId, studentSlot: slot, hasVisited: true }) });
    setRefreshKey(k => k + 1);
  };
  ...
  const renderStudentView = (isPickedUp: boolean, hasSlot: boolean, isFirst: boolean, t: any) => {
    if (!isPickedUp && isFirst) return (
      <div className="bg-gradient-to-b from-yellow-50 to-white p-10 rounded-2xl border border-yellow-200 text-center shadow-sm">
        ...
        <div className="flex gap-3 max-w-md mx-auto"><input id="s1" className="border border-yellow-300 bg-white p-3 rounded-xl flex-1 focus:ring-2 focus:ring-yellow-500 outline-none shadow-inner" placeholder="e.g. Monday 10:00 AM" /><button onClick={() => saveSlot((document.getElementById('s1') as any).value)} className="bg-yellow-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-yellow-700 shadow-md">Save</button></div>
      </div>
    );
    if (!isPickedUp && !isFirst) return (
       <div className="bg-gradient-to-b from-orange-50 to-white p-10 rounded-2xl border border-orange-200 text-center shadow-sm"><div className="w-20 h-20 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm"><Clock size={40}/></div><h4 className="font-bold text-orange-800 text-2xl mb-3">Awaiting Assignment</h4><p className="text-orange-700 max-w-md mx-auto mb-6">Your request is in the queue. Proposed time: <b>{appt?.studentSlot}</b></p></div>
    );
    if (isPickedUp && hasSlot) return (
       <div className="bg-gradient-to-b from-green-50 to-white p-10 rounded-2xl border border-green-200 text-center shadow-sm"><div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm"><CheckCircle size={40}/></div><h4 className="font-bold text-green-800 text-2xl mb-2">Confirmed!</h4><p className="text-green-700 text-lg mb-6">Matched with <b>{t.counselorName}</b> at:</p><div className="bg-white px-8 py-4 border border-green-200 rounded-xl font-mono text-xl font-bold text-green-700 shadow-sm inline-block">{appt.studentSlot}</div></div>
    );
    if (isPickedUp && !hasSlot) return (
      <div className="bg-gradient-to-b from-blue-50 to-white p-10 rounded-2xl border border-blue-200 text-center shadow-sm"><div className="w-20 h-20 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm"><AlertCircle size={40}/></div><h4 className="font-bold text-blue-800 text-2xl mb-3">Action Required</h4><p className="mb-8 text-blue-700 max-w-md mx-auto leading-relaxed">Ticket picked up! Please confirm final time.</p><div className="flex gap-3 max-w-md mx-auto"><input id="s2" className="border border-blue-300 bg-white p-3 rounded-xl flex-1 focus:ring-2 focus:ring-blue-500 outline-none shadow-inner" placeholder="e.g. Tuesday 2:00 PM" /><button onClick={() => saveSlot((document.getElementById('s2') as any).value)} className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 shadow-md">Confirm</button></div></div>
    );
  ...
  const renderCounselorView = (isPickedUp: boolean, hasSlot: boolean, t: any) => {
      return (
        <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center shadow-sm h-full flex flex-col items-center justify-center">
            <div className="mb-4 text-slate-400"><UserCheck size={40}/></div><h4 className="font-bold text-slate-700 text-xl mb-2">Appointment Status</h4>
            <div className="space-y-2 mb-6"><p className="text-slate-500">Student: <b>{t.studentName}</b></p><p className="text-slate-500">Requested Date: <b>{t.requestDate ? new Date(t.requestDate).toLocaleDateString() : 'Flexible'}</b></p></div>
            <div className={`p-4 rounded-xl border w-full max-w-md ${isPickedUp ? (hasSlot ? "bg-green-50 border-green-200 text-green-700" : "bg-blue-50 border-blue-200 text-blue-700") : "bg-orange-50 border-orange-200 text-orange-700"}`}>
                ...
                <div className="font-bold text-lg">{ !isPickedUp ? "Waiting for Pickup" : !hasSlot ? "Waiting for Student Confirmation" : `Confirmed: ${appt?.studentSlot}` }</div>
            </div>
        </div>
      )
  };
  ...
  const renderLogic = () => {
    if (!selId) return (<div className="text-gray-400 h-full flex flex-col items-center justify-center p-10 text-center border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50"><Calendar size={64} className="mb-6 opacity-20 text-violet-500"/><p className="font-medium text-lg text-gray-500">No Ticket Selected</p></div>);
    ...
    if (session.role === 'counselor') { return renderCounselorView(isPickedUp, hasSlot, t); } else { return renderStudentView(isPickedUp, hasSlot, isFirst, t); }
  };
  ...
  return (
    <Card title="Appointment Management" icon={Calendar} onClose={onClose}>
      <div className="grid grid-cols-12 gap-8 h-full">
        ...
        <div className="col-span-8 flex flex-col justify-center">{renderLogic()}</div>
      </div>
    </Card>
  );
};
      ...
      return (
        <Card title="Counselor Admin Hub" icon={Phone} onClose={onClose} accentColor="text-red-600">
            <div className="flex flex-col items-center justify-center h-full max-w-3xl mx-auto space-y-6 text-center">
                ...
                <div><h2 className="text-2xl font-bold text-gray-700">Restricted Access</h2><p className="text-gray-500 mt-2 max-w-sm mx-auto">This area is for authorized counselors only.</p></div>
            </div>
        </Card>
      )
  }
  return (
    <Card title="Counselor Admin Hub" icon={Phone} onClose={onClose} accentColor="text-red-600">
      <div className="flex flex-col items-center justify-center h-full max-w-3xl mx-auto space-y-10">
        <div className="bg-red-50 p-8 rounded-2xl w-full border border-red-100 flex items-center gap-6 shadow-sm"><div className="bg-red-100 p-4 rounded-full text-red-600 shrink-0"><Phone size={32}/></div><div className="flex-1"><h4 className="font-bold text-red-800 text-xl">Emergency Contact Information</h4><div className="flex gap-4"><div className="bg-white px-5 py-3 rounded-xl text-red-700 font-bold shadow-sm border border-red-100 flex-1 text-center text-lg">Crisis Line: 988</div><div className="bg-white px-5 py-3 rounded-xl text-red-700 font-bold shadow-sm border border-red-100 flex-1 text-center text-lg">Campus Police: 555-0199</div></div></div></div>
        <div className="text-center p-10 bg-white rounded-3xl border border-gray-200 w-full shadow-xl relative overflow-hidden">
          <div className={`absolute top-0 left-0 w-full h-2 ${isActive ? "bg-green-500" : "bg-gray-300"}`}></div><h4 className="font-bold text-gray-800 text-2xl mb-3">Counselor Availability</h4><p className="text-gray-500 mb-8 max-w-md mx-auto">You must set your status to <b>ONLINE</b> to accept new tickets.</p>
          <button onClick={onToggle} className={`group relative w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl transition-all transform hover:scale-105 active:scale-95 border-8 ${isActive ? "bg-green-500 border-green-100 shadow-green-200" : "bg-gray-100 border-gray-50"}`}><Power size={48} className={`transition-all duration-300 ${isActive ? "text-white" : "text-gray-400"}`}/></button>
          <div className={`inline-block px-6 py-2 rounded-full text-sm font-bold uppercase tracking-widest transition-colors ${isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>{isActive ? "System Online" : "System Offline"}</div>
        </div>
      </div>
    </Card>
  );
    ...
    <div className="bg-gray-50 min-h-screen font-sans text-gray-900 pb-12">
      <div className="bg-violet-900 text-white px-8 py-5 flex justify-between items-center shadow-lg sticky top-0 z-20">
        <div className="flex items-center gap-4">
            ...
            <div><h1 className="text-2xl font-bold tracking-tight">Western<span className="font-light opacity-90">Care</span></h1><p className="text-xs text-violet-200 font-medium tracking-wide uppercase">Microservices Architecture</p></div>
        </div>
        <div className="flex gap-6 items-center">
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${counselingMode ? "bg-green-500/20 border-green-400/50 text-green-300" : "bg-red-500/20 border-red-400/50 text-red-300"}`}><div className={`w-2 h-2 rounded-full ${counselingMode ? "bg-green-400 animate-pulse" : "bg-red-400"}`}></div>{counselingMode ? "COUNSELORS ONLINE" : "COUNSELORS OFFLINE"}</div>
          ...
          <div className="flex items-center gap-3 bg-violet-800/50 p-1.5 rounded-xl border border-violet-700">
              <div className="text-right hidden md:block px-2"><div className="text-[10px] font-bold text-violet-300 uppercase tracking-wider">Current User</div><div className="text-sm font-bold text-white">{session.name}</div></div>
              <div className="bg-violet-600 p-2 rounded-lg"><Users size={20}/></div>
              <select value={session.role} onChange={e => handleRoleSwitch(e.target.value as Role)} className="bg-transparent text-white text-sm font-medium focus:outline-none cursor-pointer"><option value="student" className="text-gray-900">Student</option><option value="counselor" className="text-gray-900">Counselor</option><option value="admin" className="text-gray-900">Admin</option></select>
          </div>
        </div>
      </div>
      ...
      <div className="p-8 max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 h-[700px]">
          {activeId ? (
            ...
            <div className="bg-white p-10 rounded-2xl shadow-xl border border-gray-200 h-full flex flex-col relative overflow-hidden">
              ...
              <div className="mb-10 relative z-10"><h2 className="text-3xl font-bold text-gray-800 flex items-center gap-3"><Server className="text-violet-600"/> Service Catalog</h2><p className="text-gray-500 mt-3 text-lg">Select a service to launch. Availability updates in real-time.</p></div>
                ...
                {[
                  ...
                  return (
                    <div key={item.id} className={`group relative border rounded-2xl p-6 transition-all duration-300 ${isUp ? "bg-white border-gray-200 hover:border-violet-300 hover:shadow-lg hover:-translate-y-1" : "bg-gray-50 border-gray-100 opacity-60 grayscale"}`}>
                      <div className="flex justify-between items-start mb-4"><div className={`p-3 rounded-xl ${isUp ? "bg-violet-50 text-violet-700" : "bg-gray-200 text-gray-400"}`}><item.icon size={28}/></div><span className={`text-[10px] font-bold px-3 py-1 rounded-full border ${isUp ? "bg-green-50 text-green-700 border-green-100" : "bg-red-50 text-red-700 border-red-100"}`}>{isUp ? "ONLINE" : "OFFLINE"}</span></div>
                      <h3 className="font-bold text-gray-800 text-xl mb-1">{item.name}</h3>
                      <p className="text-gray-500 text-sm mb-6">{item.desc}</p>
                      <button disabled={!isUp} onClick={() => { setActiveId(item.id as ServiceId); addLog(`${session.name} opened ${item.name}`); }} className={`w-full py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${isUp ? "bg-violet-900 text-white hover:bg-violet-800 shadow-md" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>{isUp ? "Launch" : "Unavailable"} {isUp && <Activity size={16}/>}</button>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-slate-900 text-white p-8 rounded-2xl shadow-2xl h-full flex flex-col border border-slate-700">
            ...
            {session.role !== "admin" ? (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-500 text-center"><ShieldAlert size={80} className="mb-6 opacity-20"/><p className="font-bold text-lg text-slate-400">Restricted Access</p><p className="text-sm mt-3 opacity-60 max-w-xs">You must switch to the <b>Admin Role</b> to manage service availability.</p></div>
            ) : (
                <div className="space-y-4">
                  <div className="text-xs text-slate-400 mb-6 bg-slate-800/50 p-4 rounded-xl border border-slate-700 leading-relaxed"><span className="text-violet-400 font-bold block mb-1">PROVISIONING CONTROL</span>Toggle services below to simulate system failures or maintenance. This sends real-time signals to the Service Registry.</div>
                  {["profile", "tickets", "board", "appointments", "counseling"].map(name => (
                    <div key={name} className="flex justify-between items-center bg-slate-800 p-4 rounded-xl border border-slate-700 hover:border-violet-500/50 transition-all group">
                      ...
                      <button onClick={() => handleToggleService(name)} className={`w-28 py-2 rounded-lg text-xs font-bold transition-all shadow-inner border ${registry[name]?.status === "UP" ? "bg-green-500/10 text-green-400 border-green-500/30 hover:bg-green-500/20" : "bg-red-500/10 text-red-400 border-red-500/30 hover:bg-red-500/20"}`}>{registry[name]?.status || "CONNECTING..."}</button>
                    </div>
                  ))}
                </div>
            )}
            <div className="mt-auto pt-8 border-t border-slate-700">
                ...
                <div className="h-48 bg-black/40 rounded-xl p-4 text-[10px] font-mono text-violet-200 overflow-hidden shadow-inner border border-slate-800 overflow-y-auto">
                    {systemLogs.length === 0 ? (<div className="text-slate-500 italic text-center mt-10">No activity recorded...</div>) : (
                        systemLogs.map((log, i) => (<div key={i} className="mb-2 truncate opacity-90 hover:opacity-100 transition-opacity border-b border-slate-800/50 pb-1"><span className="text-green-400">➜</span> {log}</div>))
                    )}
                </div>
            </div>
          </div>
