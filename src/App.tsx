...
const api = {
  discover: async () => {
    ...
    try {
      ...
      console.error("Registry down or sleeping", e);
      return {};
    }
  },
...
const Card = ({
  ...
  <div className="bg-white rounded-2xl shadow-xl border border-gray-200 h-full flex flex-col overflow-hidden transition-all duration-300">
    ...
    <div className="flex-1 overflow-y-auto p-6 relative bg-gray-50/30">{children}</div>
  </div>
);
...
const ServiceProfile = ({
  ...
  const save = async () => {
    ...
    try {
      await api.callService("profile", `/profile/${session.name}`, {
        method: "POST",
        body: JSON.stringify(data)
      });
    } catch (e) {
      console.warn("Backend save failed, using local state", e);
    }
      ...
      <div className="max-w-lg mx-auto space-y-8">
        ...
        <div className="space-y-5 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div>
            ...
            <input
              className="w-full border-gray-300 bg-gray-50 rounded-lg p-3 focus:ring-2 focus:ring-violet-500 outline-none border transition-all"
              value={data.name}
          ...
          <div>
            ...
            <textarea
              className="w-full border-gray-300 bg-gray-50 rounded-lg p-3 focus:ring-2 focus:ring-violet-500 outline-none border h-32 transition-all resize-none"
              value={data.bio}
          ...
          <button
            onClick={save}
            disabled={loading}
            className="w-full bg-violet-700 hover:bg-violet-800 text-white font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 shadow-md transform active:scale-95 disabled:opacity-50"
          >
            {loading ? "Saving..." : (
              <>
...
const ServiceTickets = ({
  ...
  const pickup = async (id: string) => {
    await api.callService("tickets", `/tickets/${id}/pickup`, {
      method: "POST",
      body: JSON.stringify({ counselorName: session.name })
    });
    await refresh();
  ...
  const deleteTicket = async (id: string) => {
    ...
    try {
      ...
      console.error("Ticket delete failed, reloading from server", e);
      await refresh();
      setToast("Delete failed — reloaded tickets.");
      setTimeout(() => setToast(""), 3000);
    }
  };
    ...
    <Card title="Support Tickets" icon={ShieldAlert} onClose={onClose}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-2xl border border-gray-200 h-fit shadow-sm">
          {session.role === "student" ? (
            <>
              <h4 className="font-bold text-xl mb-6 flex items-center gap-2 text-violet-800">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none mt-1"
                    value={sub}
                ...
                <div>
                  ...
                  <textarea
                    className="w-full border border-gray-300 p-3 rounded-xl h-32 focus:ring-2 focus:ring-violet-500 outline-none resize-none mt-1"
                    value={desc}
                ...
                <button
                  onClick={create}
                  className="w-full bg-violet-700 text-white py-3 rounded-xl font-bold hover:bg-violet-800 shadow-md transition-all"
                >
                  Submit Ticket
                </button>
              </div>
            ...
            <div className="text-center p-6 bg-gray-50 rounded-xl border border-gray-200">
              <div
                className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center mb-4 transition-colors ${
                  counselingMode
              ...
              <div
                className={`mt-3 px-4 py-1 rounded-full text-xs font-bold inline-block ${
                  counselingMode ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"
                }`}
              >
              ...
              <p className="text-sm text-gray-500 mt-4 leading-relaxed">
                {counselingMode
                  ? "Select an OPEN ticket from the list to assign it to yourself."
                  : "Go to the Counseling Hub (S5) to go Online."}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {tickets.map((t) => (
            <div
              key={t.id}
              className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm hover:shadow-md transition-all relative group"
            >
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1 flex gap-2">
                    ...
                    <span className="font-medium text-gray-700">
                      {t.requestDate ? new Date(t.requestDate).toLocaleString() : "ASAP"}
                    </span>
                  </div>
                  <div className="flex gap-2 mt-3 flex-wrap">
                    ...
                    <span className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded-full border border-gray-200 flex items-center gap-1">
                      <Users size={10} /> {t.studentName}
                    </span>
                    {t.counselorName && (
                      <span className="text-xs bg-blue-50 text-blue-600 px-3 py-1 rounded-full border border-blue-200 flex items-center gap-1">
                        <GraduationCap size={10} /> {t.counselorName}
                      </span>
                    )}
                  </div>
                </div>
...
const ServiceBoard = ({ session, onClose }: { session: UserSession; onClose: any }) => {
  ...
  const post = async () => {
    ...
    await api.callService("board", "/posts", {
      method: "POST",
      body: JSON.stringify({ content: text, author: authorLabel })
    });
    setText("");
    ...
    <Card title="Community Chat" icon={MessageSquare} onClose={onClose}>
      <div className="flex flex-col h-full bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
        ...
        <div className="p-4 bg-white border-t border-gray-200 flex gap-3 items-center">
          ...
          <button
            onClick={post}
            className="bg-violet-700 text-white p-3 rounded-full hover:bg-violet-800 transition-colors shadow-md hover:shadow-lg active:scale-95"
          >
            <Send size={20} />
          </button>
        </div>
...
const ServiceAppointments = ({ session, onClose }: { session: UserSession; onClose: any }) => {
  ...
  useEffect(() => {
    if (selId) {
      api
        .callService("appointments", `/appointments/${selId}`)
        .then(setAppt)
        .catch(console.error);
    }
  }, [selId, refreshKey]);
    ...
    if (!isPickedUp && isFirst)
      return (
        <div className="bg-gradient-to-b from-yellow-50 to-white p-10 rounded-2xl border border-yellow-200 text-center shadow-sm">
          ...
          <p className="mb-8 text-yellow-700 max-w-md mx-auto leading-relaxed">
            No counselor assigned yet. Please propose a time.
          </p>
          <div className="flex gap-3 max-w-md mx-auto">
            <input
              id="s1"
              className="border border-yellow-300 bg-white p-3 rounded-xl flex-1 focus:ring-2 focus:ring-yellow-500 outline-none shadow-inner"
              placeholder="e.g. Monday 10:00 AM"
            />
            <button
              onClick={() => saveSlot((document.getElementById("s1") as any).value)}
              className="bg-yellow-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-yellow-700 shadow-md"
            >
              Save
            </button>
          </div>
      ...
      return (
        <div className="bg-gradient-to-b from-orange-50 to-white p-10 rounded-2xl border border-orange-200 text-center shadow-sm">
          ...
          <p className="text-orange-700 max-w-md mx-auto mb-6">
            Your request is in the queue. Proposed time: <b>{appt?.studentSlot}</b>
          </p>
        </div>
      );
    if (isPickedUp && hasSlot)
      return (
        <div className="bg-gradient-to-b from-green-50 to-white p-10 rounded-2xl border border-green-200 text-center shadow-sm">
          ...
          <p className="text-green-700 text-lg mb-6">
            Matched with <b>{t.counselorName}</b> at:
          </p>
    ...
    if (isPickedUp && !hasSlot)
      return (
        <div className="bg-gradient-to-b from-blue-50 to-white p-10 rounded-2xl border border-blue-200 text-center shadow-sm">
          ...
          <p className="mb-8 text-blue-700 max-w-md mx-auto leading-relaxed">
            Ticket picked up! Please confirm final time.
          </p>
          <div className="flex gap-3 max-w-md mx-auto">
            <input
              id="s2"
              className="border border-blue-300 bg-white p-3 rounded-xl flex-1 focus:ring-2 focus:ring-blue-500 outline-none shadow-inner"
              placeholder="e.g. Tuesday 2:00 PM"
            />
            <button
              onClick={() => saveSlot((document.getElementById("s2") as any).value)}
              className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 shadow-md"
            >
              Confirm
            </button>
          </div>
    ...
    return (
      <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center shadow-sm h-full flex flex-col items-center justify-center">
        ...
        <div className="space-y-2 mb-6">
          ...
          <p className="text-slate-500">
            Requested Date:{" "}
            <b>{t.requestDate ? new Date(t.requestDate).toLocaleDateString() : "Flexible"}</b>
          </p>
        </div>
        ...
        <div className="col-span-4 border-r border-gray-100 pr-6 overflow-y-auto">
          ...
          <div className="space-y-3">
            {tickets.map((t) => (
              ...
              >
                <div className="font-bold text-gray-800 text-sm group-hover:text-violet-700">
                  #{t.id} {t.subject}
                </div>
                <div
                  className={`text-[10px] mt-2 font-bold uppercase inline-block px-2 py-0.5 rounded-md ${
                    t.status === "OPEN"
      ...
      >
        <div className="flex flex-col items-center justify-center h-full max-w-3xl mx-auto space-y-6 text-center">
          <div className="bg-red-50 p-6 rounded-full">
            <Lock size={64} className="text-red-300" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-gray-700">Restricted Access</h2>
            <p className="text-gray-500 mt-2 max-w-sm mx-auto">
              This area is for authorized counselors only. Students cannot toggle availability
              modes.
            </p>
          </div>
        </div>
    ...
    >
      <div className="flex flex-col items-center justify-center h-full max-w-3xl mx-auto space-y-10">
        <div className="bg-red-50 p-8 rounded-2xl w-full border border-red-100 flex items-center gap-6 shadow-sm">
          ...
          <div className="flex-1">
            ...
            <p className="text-red-600/80 text-sm mb-4">
              Provide these numbers immediately if a student is in danger.
            </p>
            <div className="flex gap-4">
              <div className="bg-white px-5 py-3 rounded-xl text-red-700 font-bold shadow-sm border border-red-100 flex-1 text-center text-lg">
                Crisis Line: 988
              </div>
              <div className="bg-white px-5 py-3 rounded-xl text-red-700 font-bold shadow-sm border border-red-100 flex-1 text-center text-lg">
                Campus Police: 555-0199
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="text-center p-10 bg-white rounded-3xl border border-gray-200 w-full shadow-xl relative overflow-hidden">
          ...
          <button
            onClick={onToggle}
            className={`group relative w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl transition-all transform hover:scale-105 active:scale-95 border-8 ${
              isActive ? "bg-green-500 border-green-100 shadow-green-200" : "bg-gray-100 border-gray-50"
            }`}
          >
            <Power
              size={48}
          ...
          <div
            className={`inline-block px-6 py-2 rounded-full text-sm font-bold uppercase tracking-widest transition-colors ${
...
export default function App() {
  ...
  useEffect(() => {
    const wakeUp = async () => {
      const bootMsg = "Waking up cloud services... (this may take 30s)";
      setToast(bootMsg);
      ...
      // auto-hide boot toast after ~8 seconds unless replaced
      setTimeout(() => {
        setToast((prev) => (prev === bootMsg ? "" : prev));
      }, 8000);
    };
    wakeUp();
  }, []);

  // Poll registry + counseling status
  useEffect(() => {
    const interval = setInterval(() => {
      api.discover().then(setRegistry);
      api
        .callService("counseling", "/status")
        .then((r) => setCounselingMode(r.active))
        .catch(() => {});
    }, 2000);
    return () => clearInterval(interval);
  }, []);

  // Auto-close open service if registry says it's DOWN
  useEffect(() => {
    if (!activeId) return;
    const svc = registry[activeId];
    if (svc && svc.status === "DOWN") {
      setActiveId(null);
      addLog(`Closed ${activeId} because registry shows DOWN`);
      setToast("Service went offline; closing tab.");
      setTimeout(() => setToast(""), 3000);
    }
  }, [registry, activeId]);
  ...
  const handleRoleSwitch = (newRole: Role) => {
    const newName =
      newRole === "student" ? "Student A" : newRole === "counselor" ? "Counselor Mike" : "Admin User";
  ...
  const handleManualWake = async (serviceId: ServiceId) => {
    const s = registry[serviceId];
    if (!s?.url) return;
    addLog(`Manual wake-up ping sent to ${serviceId}`);
    setToast(`Waking ${serviceId} in the background...`);
    try {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">
              Western<span className="font-light opacity-90">Care</span>
            </h1>
            <p className="text-xs text-violet-200 font-medium tracking-wide uppercase">
              Microservices Architecture
            </p>
          </div>
        </div>
        <div className="flex gap-6 items-center">
          {/* Counselor Status Indicator (Global) */}
          <div
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                counselingMode ? "bg-green-400 animate-pulse" : "bg-red-400"
              }`}
            ></div>
          ...
          <div className="flex items-center gap-3 bg-violet-800/50 p-1.5 rounded-xl border border-violet-700">
            ...
            >
              ...
              <option value="counselor" className="text-gray-900">
                Counselor
              </option>
      ...
      <div className="p-8 max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <>
              {activeId === "profile" && (
                <ServiceProfile
                  ...
                  onClose={() => {
                    setActiveId(null);
                    addLog(`${session.name} closed Profile`);
                  }}
                />
              )}
              {activeId === "tickets" && (
                <ServiceTickets
                  ...
                  onClose={() => {
                    setActiveId(null);
                    addLog(`${session.name} closed Tickets`);
                  }}
                />
              )}
              {activeId === "board" && (
                <ServiceBoard
                  ...
                  onClose={() => {
                    setActiveId(null);
                    addLog(`${session.name} closed Board`);
                  }}
                />
              )}
              {activeId === "appointments" && (
                <ServiceAppointments
                  ...
                  onClose={() => {
                    setActiveId(null);
                    addLog(`${session.name} closed Appointments`);
                  }}
                />
              )}
              {activeId === "counseling" && (
                <ServiceCounseling
                  session={session}
                  isActive={counselingMode}
                  onToggle={toggleCounseling}
                  onClose={() => {
                    setActiveId(null);
                    addLog(`${session.name} closed Counseling`);
                  }}
                />
              )}
            </>
            ...
            <div className="bg-white p-10 rounded-2xl shadow-xl border border-gray-200 h-full flex flex-col relative overflow-hidden">
              ...
              <div className="mb-10 relative z-10">
                <h2 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
                  <Server className="text-violet-600" /> Service Catalog
                </h2>
                <p className="text-gray-500 mt-3 text-lg">
                  Select a service to launch. Availability updates in real-time.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 overflow-y-auto relative z-10 pb-4">
                {[
                  ...
                  },
                  {
                    ...
                    name: "Counselor Hub",
                    icon: Phone,
                    desc: "Staff Access Only"
                  }
                ].map((item) => {
                  const s = registry[item.id];
                  const isUp = s?.status === "UP";
                  const statusLabel = s?.status || "DOWN";

                  return (
                    <div
                      key={item.id}
                      className={`group relative border rounded-2xl p-6 transition-all duration-300 ${
                        isUp
                          ? "bg-white border-gray-200 hover:border-violet-300 hover:shadow-lg hover:-translate-y-1"
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div
                          className={`p-3 rounded-xl ${
                            isUp ? "bg-violet-50 text-violet-700" : "bg-gray-200 text-gray-400"
                          }`}
                        >
                          <item.icon size={28} />
                        </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          disabled={!isUp}
                          onClick={() => {
                            setActiveId(item.id as ServiceId);
                            addLog(`${session.name} opened ${item.name}`);
                          }}
                          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                            isUp
                              ? "bg-violet-900 text-white hover:bg-violet-800 shadow-md"
                        ...
                        {/* Wake button: sends background ping only, no navigation */}
                        {s?.url && (
                          <button
                            type="button"
                            onClick={() => handleManualWake(item.id as ServiceId)}
                            title="Send a background ping to warm this cloud service before launching."
        ...
        <div className="lg:col-span-4">
          <div className="bg-slate-900 text-white p-8 rounded-2xl shadow-2xl h-full flex flex-col border border-slate-700">
            <h3 className="font-bold mb-8 flex items-center gap-3 border-b border-slate-700 pb-6 text-xl">
              <Activity className="text-violet-400" /> Admin Provisioning
            </h3>
            ...
            {session.role !== "admin" ? (
              <div className="flex-1 flex flex-col items-center justify-center text-slate-500 text-center">
                ...
                <p className="text-sm mt-3 opacity-60 max-w-xs">
                  You must switch to the <b>Admin Role</b> to manage service availability.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="text-xs text-slate-400 mb-6 bg-slate-800/50 p-4 rounded-xl border border-slate-700 leading-relaxed">
                  ...
                  Toggle services below to simulate system failures or maintenance. This sends
                  real-time signals to the Service Registry.
                </div>
                {["profile", "tickets", "board", "appointments", "counseling"].map((name) => (
                  <div
                    key={name}
                    className="flex justify-between items-center bg-slate-800 p-4 rounded-xl border border-slate-700 hover:border-violet-500/50 transition-all group"
                  >
                    <span className="capitalize font-medium text-slate-200 group-hover:text-white transition-colors">
                      {name}
                    </span>
                    <button
                      onClick={() => handleToggleService(name)}
                      className={`w-28 py-2 rounded-lg text-xs font-bold transition-all shadow-inner border ${
                        registry[name]?.status === "UP"
                          ? "bg-green-500/10 text-green-400 border-green-500/30 hover:bg-green-500/20"
                          : "bg-red-500/10 text-red-400 border-red-500/30 hover:bg-red-500/20"
                      }`}
                    >
                      {registry[name]?.status || "DOWN"}
                    </button>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-auto pt-8 border-t border-slate-700">
              ...
              <div className="h-48 bg-black/40 rounded-xl p-4 text-[10px] font-mono text-violet-200 shadow-inner border border-slate-800 overflow-y-auto">
                {systemLogs.length === 0 ? (
                  ...
                  systemLogs.map((log, i) => (
                    <div
                      key={i}
                      className="mb-2 truncate opacity-90 hover:opacity-100 transition-opacity border-b border-slate-800/50 pb-1"
                    >
